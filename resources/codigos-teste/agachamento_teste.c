...
int mainteste(void)
{
    ...
    PhysicsBody platformRight3 = CreatePhysicsBodyRectangle((Vector2){screenWidth * 1.50f, screenHeight * 0.7f}, 100, 20, 10);
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        if (btnAction)
        {
            ...
            while (!WindowShouldClose()) // Detect window close button or ESC key
            {
                ...
                if(IsKeyPressed(KEY_DOWN) && !isShortened){
                    abaixar(&body);
                    isShortened = 1;
                }
